...
def Mirror(src,label=None):
    ...
    keypoints = [keypoints[18],keypoints[19],keypoints[20],
...
def Crop(src, label, pad_color, max_pad_ratio=0.4):
    """
    item = {
             "img_name":save_name,  
             "keypoints":save_keypoints, relative position
    ...
    pad_ratio = random.uniform(max_pad_ratio/2, max_pad_ratio)
...
def AID(img, label):
    ...
    half_size = int(random.uniform(20/192,36/192)*(h+w)/2)
...
def AID2(img, label):
    ...
    half_size = int(random.uniform(5/192,10/192)*(h+w)/2)
